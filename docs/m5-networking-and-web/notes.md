...
You type a name, hit Enter, and a page from a computer on the other side of the planet appears in under a second. This module traces *exactly* what happens in those moments the address lookup (**DNS**), the **request and response**, the padlock (**HTTPS**), and why it's all so fast (**CDNs**). By the end, the web isn't magic but it's a sequence you can name.


**## Internet vs web are not the same thing**
People say them interchangeably, but:

- The **internet** is the global network of networks ie the roads. (M1's "data center → cloud" computers, all wired together.)
...
For a message to reach the right computer, each one needs a unique **IP address** like **`172.66.147.243`**. The classic style (IPv4) is four numbers; a newer style (IPv6) is much longer and far more plentiful like **`2001:0db8:85a3:0000:0000:8a2e:0370:7334`**. Without an address, a reply has nowhere to go.

**## DNS is considered the internet's phone book**
You don't memorize **`172.66.147.243`**; you type **`thinktechconsulting.com`**. The system that turns the **name** into the **number** is **DNS** (Domain Name System). Every time you open a site, your computer quietly asks DNS "what's the IP for this name?" first. (You'll do that lookup by hand in the lab with **`dig`**.)
...
Your device is the **client**: it sends a **request**. A **server** (a computer whose job is to serve pages) sends back a **response**, starting with a status like **`100`** (informational), **`200`** (success), **`300`** (redirect), **`400`** (client_site_error), **`500`** (server_site_error). That request→response loop is the heartbeat of the web, and **HTTP** is the language they speak.

**## HTTPS & TLS, what the padlock means**
Plain **HTTP** is like a postcard: anyone who handles it along the way can read it. **HTTPS** is HTTP wrapped in **TLS encryption** ie it scrambles the connection so only you and the server can read it. The **padlock** in your address bar means HTTPS is on.

How does your browser know the server is *really* **`thinktechconsulting.com`** and not an impostor? A trusted authority issues the site a **TLS certificate** a verifiable ID card. Your browser checks it automatically; you can inspect it yourself (who it's for, who issued it, and its valid dates). Modern reality: **HTTPS is now everywhere**, and browsers warn loudly on plain HTTP.
...
A big website isn't one computer in one place. It's copies cached on many servers worldwide, a **CDN** (content delivery network) so you connect to a nearby one instead of a distant origin. That's why a site loads quickly whether you're in Lagos or London. Real example: **`thinktechconsulting.com`** is actually served by **Cloudflare**, a CDN — you'll see "server: cloudflare" in the lab. This is "the cloud" doing its job (much more in M8).
...
**## See it yourself**
In your Codespace: **`dig https://thinktechconsulting.com`** (watch the name become an IP), then **`curl -I https://https://thinktechconsulting.com/`** (see the **`200`** response come back over HTTPS). In a browser, click the **padlock** to read the site's certificate.
...
**2. When you type a name like **`thinktechconsulting.com`**, what turns that name into a number the computer can reach?**
...
??? success "Show answer"
    Because of a **CDN** (content delivery network) — copies of the site cached on many servers worldwide, so you connect to a *nearby* one instead of a distant origin. **`thinktechconsulting.com`** is actually served by **Cloudflare**, a CDN.
