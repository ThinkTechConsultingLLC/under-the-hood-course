...
```text
$ top or $ htop --> linux / mac
PS C:\user\TTC> tasklist ----> windows
```
...
For a visual breakdown of your windows system's overall RAM health and the processes eating up the most resources, check out: https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.management/get-process?view=powershell-7.6
```
...
```text
$ sleep 300 & ---> linux / mac
PS C:\user\TTC> start calc.ex ----> windows
```
...
PS C:\user\TTC> tasklist
PS C:\user\TTC> taskkill 4567
PS C:\user\TTC> tasklist
```
...
#Windows commands
PS C:\user\TTC> type nul > demo.txt
PS C:\user\TTC> dir demo.txt
```
...
#Windows commands
icacls to Access Permissions

Example:
Grant Full Control:            PS C:\user\TTC> icacls "C:\path\to\file.txt" /grant UserName:F
Grant Read-Only Access:        PS C:\user\TTC> icacls "C:\path\to\file.txt" /grant UserName:R
Deny Access:                   PS C:\user\TTC> icacls "C:\path\to\file.txt" /deny UserName:F
Reset to Default Permissions:  PS C:\user\TTC> icacls icacls "C:\path\to\folder" /reset /t
```
